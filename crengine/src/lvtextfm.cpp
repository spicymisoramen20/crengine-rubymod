...
enum RubyToggleObscureStyle {
    ...
    // Skip <rt> ink; fill the slot with a fine light Bayer fog (e-ink-friendly).
    RUBY_TOGGLE_OBSCURE_FOG = 2,
};

static bool g_ruby_toggle_mode = false;
static int g_ruby_toggle_obscure = RUBY_TOGGLE_OBSCURE_HIDDEN;
// Fog density: 0 = no veil, 100 = fully opaque.
...
enum RubyToggleFogPattern {
    ...
    RUBY_FOG_MULTI4 = 5,
    // Continuous midtone fill (no speckles). Softest on panels that hold grays.
    RUBY_FOG_SOFT_GRAY = 6,
    // 8 midtone levels, nearest-level quantization (minimal residual grain).
    RUBY_FOG_MULTI8 = 7,
    RUBY_FOG_BAYER32 = 8,
    RUBY_FOG_BLUE_NOISE32 = 9,
};
...
{
    if (style < RUBY_TOGGLE_OBSCURE_HIDDEN || style > RUBY_TOGGLE_OBSCURE_FOG)
...
void rubyToggleSetFogParams(int falloff, int roundness, int pattern)
{
    ...
    if (roundness > 64)
        roundness = 64;
    if (pattern < RUBY_FOG_BAYER8 || pattern > RUBY_FOG_BLUE_NOISE32)
...
// density / tone near the boundary; roundness uses a rounded-rect silhouette.
...
static float rubyToggleSmoothstep01(float t)
{
    if (t <= 0.f)
        return 0.f;
    if (t >= 1.f)
...
// Map ink amount 0..1 → packed RGB gray (0=black, 1=white).
static lUInt32 rubyToggleGrayRGB(float white_amount)
{
    if (white_amount < 0.f)
        white_amount = 0.f;
    if (white_amount > 1.f)
        white_amount = 1.f;
    int g = (int)(white_amount * 255.f + 0.5f);
    if (g < 0)
        g = 0;
    if (g > 255)
        g = 255;
    return ((lUInt32)g << 16) | ((lUInt32)g << 8) | (lUInt32)g;
}
...
static const unsigned short BAYER32[32][32] = {
...
static const unsigned char BN32[32][32] = {
    { 134,  62, 200,   1, 177,  32, 245,  11, 184,  33, 251,   3, 190,  44, 130,  14, 195,  46, 171,   1, 240,  48, 178,   7, 246,  57, 185,   2, 252,  59, 228,  10 },
    {  87, 254, 120, 230,  90, 168, 123, 237, 111, 175,  81, 243, 114, 181,  84, 158, 117, 225,  87, 163, 121, 231, 108, 169,  78, 238, 112, 215,  81, 154, 114, 220 },
    { 219,  27, 158,  37, 224,  28, 205,  38, 143,  29, 210,  39, 148,  18, 215,  52, 153,  19, 219,  53, 200,  20, 139,  54, 205,  24, 144,  34, 210,  25, 148,  35 },
    {  71, 135, 104, 201,  73, 140,  65, 206,  97, 145,  67, 211, 100, 191,  69, 131, 102, 196,  72, 136, 126, 202,  95, 141,  65, 207,  97, 187,  67, 254, 100, 192 },
    { 191,  45, 130,  15, 235,  47, 173,   7, 241,  48, 180,   9, 248,  58, 186,   5, 253,  60, 230,  10, 168,  62, 236,   6, 174,  41, 242,  12, 181,  42, 158,   6 },
    { 179,  42, 247,  13, 185,  43, 252,   7, 228,  44, 166,  14, 234,  46, 173,   4, 241,  56, 180,   8, 157,  58, 223,   4, 162,  60, 229,  11, 167,  39, 236,   5 },
    { 111, 213,  80, 151, 113, 218, 103, 156,  72, 223, 105, 204,  75, 142, 108, 209,  78, 147,  68, 214, 101, 152,  71, 218, 103, 199,  73, 138, 105, 204,  75, 143 },
    { 140,  28, 206,  50, 144,  17, 210,  51, 149,  18, 131,  52, 196,  19, 136,  63, 201,  23, 140,  32, 206,  24, 187,  34, 253,  25, 192,  35, 131,  30, 197,  47 },
    {  92, 172, 126, 241,  95, 179,  83, 247, 116, 186,  86, 253, 120, 229,  90, 167, 123, 235,  92, 174,  81, 242, 114, 180,  84, 157, 117, 224,  86, 162, 120, 230 },
    { 236,  56, 174,   2, 243,  57, 181,   9, 158,  58, 225,   0, 163,  38, 231,  11, 169,  40, 237,   3, 215,  41, 153,  13, 220,  42, 159,   0, 226,  54, 164,   8 },
    {  75, 143, 108, 209,  99, 148,  69, 214, 101, 152,  71, 135, 103, 200,  73, 139, 105, 205,  97, 144,  67, 210,  99, 191,  69, 130, 101, 196,  71, 135, 126, 201 },
    { 197,  20, 137,  63, 202,  23, 141,  33, 248,  25, 187,  34, 208,  26, 193,  45, 132,  31, 198,  47, 137,  15, 243,  49, 182,  16, 249,  59, 188,  21, 127,  61 },
    { 124, 199,  94, 176,  64, 244,  96, 183,  66, 250, 118, 189,  88, 128, 121, 233,  91, 170, 125, 239,  94, 177,  83, 245, 116, 183,  85, 251, 119, 228,  88, 166 },
    { 164,  39, 232,  12, 170,  40, 149,   6, 216,  41, 154,  13, 221,  53, 160,   3, 227,  54, 165,   8, 145,  55, 212,   4, 150,  36, 217,  11, 155,  37, 222,   5 },
    { 107, 208,  77, 146, 110, 213,  80, 151,  70, 218, 102, 156,  72, 138, 105, 203,  75, 142, 107, 208,  98, 147,  68, 213, 101, 152,  70, 133, 103, 199,  73, 138 },
    { 128,  30, 194,  45, 134,  29, 199,  48, 177,  16, 246,  49, 184,  17, 252,  59, 190,  22, 129,  61, 233,  22, 171,  63, 240,  22, 178,  42, 247,  30, 185,  43 },
    {  89, 166, 123, 234,  92, 172, 126, 240, 113, 179,  83, 247, 116, 186,  86, 161, 119, 229,  89, 167, 123, 235, 111, 173,  80, 242, 113, 219,  83, 157, 117, 224 },
};

    switch (pattern) {
    case RUBY_FOG_BAYER16:
        return ((int)BAYER16[ay & 15][ax & 15] * 100) / 256;
    case RUBY_FOG_BAYER32:
        return ((int)BAYER32[ay & 31][ax & 31] * 100) / 1024;
    case RUBY_FOG_BLUE_NOISE:
        return ((int)BN16[ay & 15][ax & 15] * 100) / 256;
    case RUBY_FOG_BLUE_NOISE32:
        return ((int)BN32[ay & 31][ax & 31] * 100) / 256;
...
static void rubyToggleDrawFog(LVDrawBuf * buf, int x0, int y0, int x1, int y1)
{
    if (!buf || x1 <= x0 || y1 <= y0)
        return;
    int intensity = g_ruby_toggle_dither_intensity;
    if (intensity <= 0)
        return;
    if (intensity > 100)
        intensity = 100;
    ...
    // Multi8: skip level 0; 1..7 denser ink.
    static const lUInt32 GRAY8[8] = {
        0xFFFFFF, 0xEEEEEE, 0xCCCCCC, 0xAAAAAA,
        0x888888, 0x555555, 0x333333, 0x000000
    };

    const lUInt32 black = 0x000000;
    const bool soft_tone = (pattern == RUBY_FOG_SOFT_GRAY || pattern == RUBY_FOG_MULTI8);
    ...
    for (int ay = y0; ay < y1; ay++) {
        for (int ax = x0; ax < x1; ax++) {
            ...
            if (soft_tone)
                coverage = rubyToggleSmoothstep01(coverage);

            int local_i = (int)((float)intensity * coverage + 0.5f);
            if (local_i <= 0)
                continue;

            if (pattern == RUBY_FOG_SOFT_GRAY) {
                // Continuous gray: intensity*coverage = ink, no speckles.
                float ink = (float)local_i / 100.f;
                if (ink < 0.004f)
                    continue;
                if (ink > 1.f)
                    ink = 1.f;
                buf->FillRect(ax, ay, ax + 1, ay + 1, rubyToggleGrayRGB(1.f - ink));
                continue;
            }

            if (pattern == RUBY_FOG_MULTI8) {
                // Nearest of 8 levels — smoother than B/W dither; no residual grain.
                float scaled = (float)local_i * 7.f / 100.f;
                int lo = (int)(scaled + 0.5f);
                if (lo < 1)
                    continue;
                if (lo > 7)
                    lo = 7;
                buf->FillRect(ax, ay, ax + 1, ay + 1, GRAY8[lo]);
                continue;
            }
...
static void rubyToggleDrawStandIn(LVDrawBuf * buf, int x0, int y0, int x1, int y1)
{
...
struct RubyToggleStandInRect {
    ...
    int y1;
    bool valid;
    RubyToggleStandInRect()
        : x0(0), y0(0), x1(0), y1(0), valid(false) {}
};
                ...
                if (word->flags & LTEXT_WORD_IS_IMAGE)
                {
                    ...
                    if (node) {
                        ...
                        if (is_vertical) {
                            ...
                            if (ruby_standin) {
                                rubyToggleAccumulateStandIn(ruby_standin_groups,
                                        (ldomNode *)srcline->object,
                            ...
                            if (ruby_standin)
                                rubyToggleAccumulateStandIn(ruby_standin_groups,
                                        (ldomNode *)srcline->object,
                                        xx, yy, xx + (int)word->width, yy + (int)word->o.height);
                            else {
                ...
                else
                {
                    ...
                    */
                    ...
                    }
                    {
                        ...
                        // paint a bar/fog stand-in or skip ink for suppressed <rt>.
                        int box_w = 0;
                        int box_h = 0;
                        if (ruby_standin && font) {
                            if (is_vertical) {
                                box_w = font->getSize();
                        ...
                        if (ruby_hide_ink) {
                            if (ruby_standin && !vert_skip_draw && font)
                                rubyToggleAccumulateStandIn(ruby_standin_groups,
                                        (ldomNode *)srcline->object,
