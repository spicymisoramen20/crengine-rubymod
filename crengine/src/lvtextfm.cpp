...
enum RubyToggleObscureStyle {
    ...
    // Skip <rt> ink; fill the slot with soft gray fog (e-ink midtones).
...
static int g_ruby_toggle_dither_intensity = 10;
...
// Soft-gray Fog: continuous midtone fill (no dither speckles). Intensity ≈
// % ink in the core; falloff smoothsteps density at the edge; roundness
// uses a rounded-rect silhouette.
...
static void rubyToggleDrawFog(LVDrawBuf * buf, int x0, int y0, int x1, int y1)
{
    ...
    for (int ay = y0; ay < y1; ay++) {
        for (int ax = x0; ax < x1; ax++) {
            ...
            float ink = ((float)intensity / 100.f) * coverage;
            if (ink < 0.004f)
                continue;
            if (ink > 1.f)
                ink = 1.f;
